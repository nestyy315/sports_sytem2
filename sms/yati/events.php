...
if (isset($_GET['event_image_id'])) {
    ...
    if ($event && $event['image']) {
        ...
        $imageData = base64_decode($event['image']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['event_name'])) {
        ...
        $query = $conn->prepare("INSERT INTO events (event_name, description, event_date, time, location, facilitator, image) VALUES (:event_name, :description, :event_date, :event_time, :location, :facilitator, :image)");
            ...
            <?php foreach ($events as $event): ?>
                <div class="col-md-4 mb-4" id="event-card-<?= $event['event_id'] ?>" class="event-card">
                    <div class="card shadow-sm" onclick="showEventDetails(<?= $event['event_id'] ?>)">
                        <img src="data:image/png;base64,<?= $event['image'] ?>" class="card-img-top" alt="Event Image">
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($event['event_name']) ?></h5>
                            <p class="card-text text-truncate"><?= htmlspecialchars($event['description'] ?? 'No description available.') ?></p>
                        </div>
                    </div>
                </div>
            <?php endforeach; ?>
